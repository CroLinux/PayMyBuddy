...
MySQL, Java and Maven are configured on your computer, you need first to implement the database, create the tables and inserts some data. For that, please execute the script « /PayMyBuddyApp/src/main/resources/database/paymybuddyDB.sql ».

( On a Mac: mysql -u root -p < /Users/***your_user***/git/PayMyBuddy/PayMyBuddyApp/src/main/resources/database/paymybuddyDB.sql )

Import this project in your IDE and start the application. Open a browser to this URL: http://localhost:8080/login to access to the login page and log-in using an existing account (user1@mail.com or user2@mail.com, both password is ‘password’) or register a new one.
